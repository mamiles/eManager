...
main(int argc, char **argv)
{
	if(getenv("JAVA_HOME") == NULL) {
		char *emHome = getenv("EM_HOME");
		if(emHome != NULL) {
			int initLen = strlen(emHome);
			char *jHome = new char[initLen + 50];
			sprintf(jHome, "JAVA_HOME=%s/3rdparty/java/jre", emHome);
			putenv(jHome);
		} 
	}
	...
	buf = (char *) MemAlloc(strlen(args.classpath) + strlen(s) + 2);
	...
	classname = (char *)env->GetStringUTFChars( mainClassName, 0);
	if (classname == NULL) {
	    env->ExceptionDescribe();
	    goto leave;
	}
	mainClass = LoadClass( env, classname);
	env->ReleaseStringUTFChars( mainClassName, classname);
    } else {
	mainClass = LoadClass( env, classname);
    }
    if (mainClass == NULL) {
    ...
    mainID = env->GetStaticMethodID( mainClass, "main",
				       "([Ljava/lang/String;)V");
    if (mainID == NULL) {
	if (env->ExceptionOccurred()) {
	    env->ExceptionDescribe();
...
NewPlatformStringArray(JNIEnv *env, char **strv, int strc)
{
    ...
    NULL_CHECK0(cls = (jclass) env->FindClass( "java/lang/String"));
    NULL_CHECK0(ary = (jobjectArray) env->NewObjectArray( strc, cls, 0));
    for (i = 0; i < strc; i++) {
...
#define MAIN_CLASS "Main-Class"
	...
	if (attr != 0) {
	    ...
	    result = (jstring) env->CallObjectMethod( attr, mid, str);
	}
    }
    return result;
}
...
TranslateDashJArgs(int *pargc, char ***pargv)
{
    ...
    char **nargv = (char **) MemAlloc((nargc + 1) * sizeof(char *));
    int i;
...
GetJREPath(char *path, jint pathsize)
{
	...
	if(JAVA_HOME == NULL) {
    	fprintf(stderr, "Error: JAVA_HOME is not set\n");
		exit(1);
	}
	sprintf(path, "%s", JAVA_HOME);
...
GetXUsagePath(char *buf, jint bufsize)
{
    ...
    dladdr(dlsym(dlopen(JVM_DLL, RTLD_NOW + RTLD_GLOBAL),
		 "JNI_CreateJavaVM"), &dlinfo);
