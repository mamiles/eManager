 ...
 * and spot microagent on a given agent.  It is used by the rulebase API
 * samples to demonstrate how a console application interact with an agent.
 * <p>
 * The agent name and console parameters are specified through the constructor
 * to setup which agent to interact with on which RV session.
 */
class RBISampleConsole implements RBISampleConstant,
	AgentMonitorListener, MicroAgentListMonitorListener, ErrorExceptionListener
{
	...
	private MicroAgentID _maidSpot;
	 ...
	 */
	public RBISampleConsole(String args[])
	{
		String agentName = null;
		String hawkDomain = "";
		String rvService = "7475";
		String rvNetwork = ";";
		String rvDaemon = "tcp:7475";
     ...
     *                  See the TIBCO Rendezvous documentation for more information.
     */
	public RBISampleConsole(String agentName, String hawkDomain, String rvService, String rvNetwork, String rvDaemon)
	{
		create(agentName, hawkDomain, rvService, rvNetwork, rvDaemon);
	}
	 ...
	 void create(String agentName, String hawkDomain, String rvService, String rvNetwork, String rvDaemon)
	{
		...
		println("Hawk Domain: "+hawkDomain);
		println("RV Service: "+rvService);
		println("RV Network: "+rvNetwork);
		println("RV Daemon: "+rvDaemon);
		println(" ");
	 ...
	 */
	MicroAgentID getSpotMicroagentID() { return _maidSpot; }
	...
	void addMicroagent(MicroAgentID mID)
	{
		...
		if (mID.getName().equals(SPOT_MICROAGENT_NAME))
		{
			_maidSpot = mID;
		}
	...
	public synchronized void onAgentAlive(AgentMonitorEvent event)
	{
		...
		if ( !agntInst.getAgentID().getName().equals(_agentName) )
			return;
		println("Found agent: "+_agentName);
